...
headers_info = ["part_type","lot_id","part_type","x","y","site_num_unpair","test_time"]
...
for record in root.iter():
	...
	if(record.tag == 'Prr'):
		...
		info_array = [part_type,lot_id,part_type,x,y,site_num_unpair,test_time]
		##Two device mode
		...
		if(two_device_code == '1'):
			...
			info_array_2 = [part_type,lot_id,part_type,x1,y1,site_num_unpair,test_time]
			info_array_1 = []
			count_2 = count_1 + 1
	...
	if(record.tag == 'Ptr'):
		...
		if(two_devices == False):
			...
			if(test_site_unpair == site_num_unpair):
				record_result = float(record.attrib['RESULT'])
		...
		if(two_devices == True):
			...
			if(site_num == '0'):
				...
				test_result_1 = float(record.attrib['RESULT'])
				array_device_1.append(test_result_1)
				length_1 = length_1 + 1
			if(site_num == '1'):
				# test_result_2 = float(record.get('RESULT'))
				test_result_2 = float(record.attrib['RESULT'])
	...
	if(record.tag == 'Eps'): #End of Record
		...
		if(two_devices == True):
			length_1 = 0
			length_2 = 0
			hash_data[count_1] = array_device_1
			hash_data[count_2] = array_device_2
			hash_data[count_1] + info_array
			[hash_data[count_2]] + info_array_2
			#Switch Off the switch mode
			two_devices = False	
			count_1 = 0
			count_2 = 0
		else:
			hash_data[device_count] = array_device
			[hash_data[device_count]] + info_array
			array_device = []
